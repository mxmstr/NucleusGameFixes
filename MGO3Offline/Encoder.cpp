...
public:
    Encoder(std::vector<uint8_t>& static_key = std::vector<uint8_t>(), std::vector<uint8_t>& crypto_key = std::vector<uint8_t>()) {
        if (static_key.empty()) {
    ...
    void __init_session_blowfish__(std::vector<uint8_t>& crypto_key = std::vector<uint8_t>()) {
        if (!crypto_key.empty()) {
            __crypto_key = crypto_key;
        }
    ...
    std::vector<uint8_t> encode(Json::Value& _command) {
        ...
        if (command["data"].isObject()) {
            ...
            if (command["session_crypto"].asBool()) {
                ...
                std::vector<uint8_t> encrypted_data = __encipher__(__session_blowfish, std::vector<uint8_t>(command["data"].asString().begin(), command["data"].asString().end()));
